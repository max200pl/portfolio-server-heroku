...
async function httpGoogleAuth(req, res, next) {
  return res
    .status(200)
    .json({ message: "Success Google Auth", user: req.session.user });
}

async function httpAuthGitHub(req, res, next) {
  return res
    .status(200)
    .json({ message: "Success GitHub Auth", user: req.session.user });
}

async function httpAuthForm(req, res, next) {
  return res
    .status(200)
    .json({ message: "Success Form Auth", user: req.session.user });
}
